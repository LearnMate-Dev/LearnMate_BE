...
public interface EmotionRepository extends JpaRepository<Emotion, Long> {
    ...
    @Query("SELECT new LearnMate.dev.model.dto.response.report.ReportResponse$EmotionDto(e.emotion, COUNT(e)) " +
            "FROM Emotion e " +
            "JOIN e.diary d " +
    ...
    @Query("SELECT new LearnMate.dev.model.dto.response.report.ReportResponse$EmotionOnDayDto(e.emotion, e.createdAt) " +
            "FROM Emotion e " +
            "JOIN e.diary d " +
            "WHERE d.user.id = :userId " +
            "AND DATE(e.createdAt) >= :startDate " +
